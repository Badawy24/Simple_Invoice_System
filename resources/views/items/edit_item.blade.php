...
<div class="breadcrumb-header justify-content-between">
    <div class="my-auto">
        <div class="d-flex">
            <h4 class="content-title mb-0 my-auto">تسجيل أصناف </h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ تعديل الاصناف </span>
        </div>
    </div>
                    ...
                    <table class="table text-md-nowrap" id="example1">
                        <thead>
                            <tr>
                                ...
                                <th class="wd-15p border-bottom-0">سعر البيع</th>
                                <th class="wd-15p border-bottom-0">تعديل الصنف</th>
                                <th class="wd-15p border-bottom-0">حذف الصنف</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php $i = 1; ?>
                            ...
                            <tr>
                                ...
                                <td>
                                    <a class="modal-effect btn btn-md btn-info" data-effect="effect-scale" data-edit_id="{{ $item->ItemID }}" data-edit_item_name="{{ $item->ItemName }}" data-edit_item_purch_price="{{ $item->PurchPrice }}" data-edit_item_sale_price="{{ $item->SalePrice }}" data-toggle="modal" href="#exampleModal4" title="تعديل {{$item->ItemName}}">
                                        تعديل
                                    </a>
                                </td>
                                <td>
                                    <a class="modal-effect btn btn-md btn-danger" data-effect="effect-scale" data-delete_id="{{ $item->ItemID }}" data-delete_item_name="{{ $item->ItemName }}" data-delete_item_purch_price="{{ $item->PurchPrice }}" data-delete_item_sale_price="{{ $item->SalePrice }}" data-toggle="modal" href="#exampleModal5" title="حذف {{$item->ItemName}}">
                                        حذف
                                    </a>
                                </td>
                            </tr>
...
<script>
    $('#exampleModal4').on('show.bs.modal', function(event) {
        ...
        var edit_item_name = button.data('edit_item_name')
        var edit_item_purch_price = button.data('edit_item_purch_price')
        var edit_item_sale_price = button.data('edit_item_sale_price')
        var modal = $(this)
        modal.find('.modal-body #edit_id').val(edit_id);
        modal.find('.modal-body #edit_item_name').val(edit_item_name);
        modal.find('.modal-body #edit_item_purch_price').val(edit_item_purch_price);
        modal.find('.modal-body #edit_item_sale_price').val(edit_item_sale_price);
    })
</script>
<script>
    $('#exampleModal5').on('show.bs.modal', function(event) {
        ...
        var delete_item_name = button.data('delete_item_name')
        var delete_item_purch_price = button.data('delete_item_purch_price')
        var delete_item_sale_price = button.data('delete_item_sale_price')
        var modal = $(this)
        modal.find('.modal-body #delete_id').val(delete_id);
        modal.find('.modal-body #delete_item_name').val(delete_item_name);
        modal.find('.modal-body #delete_item_purch_price').val(delete_item_purch_price);
        modal.find('.modal-body #delete_item_sale_price').val(delete_item_sale_price);
